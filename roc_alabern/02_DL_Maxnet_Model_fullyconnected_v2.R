...
prinResults <- function(results, pos) {
  message("")
  message(pos)
  message(results[[pos]]["params"])
  
  message(results[[pos]]["acc_train"])
  message(results[[pos]]["acc_test"])
}

results = list()

for (n_neurons in c(10,15,20,25,30)) {
  pos = length(results) + 1
  results[[pos]] = list()
  
  results[[pos]]["params"] = list("n_neurons"=n_neurons)
    
  data <- mx.symbol.Variable("data")
  fc1 <- mx.symbol.FullyConnected(data, name="fc1", num_hidden=n_neurons)
  ...
  results[[pos]]["model"] = model
  
  results[[pos]]["confMat_train"] = confMat_train
  results[[pos]]["confMat_test"] = confMat_test
  
  results[[pos]]["acc_train"] = acc_train
  results[[pos]]["acc_test"] = acc_test
  
  prinResults(results, pos)
}

for (i in 1:length(results)) {
  message(results[[i]]["acc_test"])
}
